...
function findCutPointRecent(messages: AgentMessage[], keepTokens: number): number {
  ...
  for (let i = messages.length - 1; i >= 0; i--) {
    ...
    if (accumulated + tokens > keepTokens) {
      return snapToProtocolBoundary(messages, i + 1);
    }
    accumulated += tokens;
  }
  return 0;
...
function findBookendCutRange(
  ...
  headEnd = snapToProtocolBoundary(messages, headEnd);
...
function applyTruncationStrategy(
  ...
  switch (strategy) {
    case "keep-recent": {
      ...
      const preserveCurrentUser = currentUserIndex >= 0 && currentUserIndex < cutIndex;
      const removed = messages
        .slice(0, cutIndex)
        .filter((_, index) => !preserveCurrentUser || index !== currentUserIndex);
      const kept = [
        ...(preserveCurrentUser ? [messages[currentUserIndex]!] : []),
    ...
    case "summarize-all": {
      // Remove everything except the latest user message. During a tool loop,
      // the final message is normally a tool result, not a user message.
      if (messages.length <= 1) return null;
      const lastUserIdx = latestUserIndex(messages);
      if (lastUserIdx < 0) return null;
      const removed = messages.filter((_, index) => index !== lastUserIdx);
      const notice = createTruncationNotice(removed.length, estimateTotalTokens(removed));
...
export default function autoCompact(pi: ExtensionAPI) {
  ...
  pi.on("context", async (event, ctx) => {
    ...
    const measuredTokens = getTokenUsage(ctx);
    lastEstimatedTokens = Math.max(estimatedTokens, measuredTokens);

    if (lastEstimatedTokens > cachedAutoCompactLimit && !pendingCompaction) {
  ...
  pi.on("turn_end", async (event, ctx) => {
    // Pi emits `toolCall`, not Anthropic's `tool_use`. More importantly,
    // turn_end already supplies authoritative results for the completed batch.
    // Upstream v0.2.4 checks `tool_use`, so its advertised mid-turn path never
    // runs on current Pi.
    if (!completedToolBatch(event) || pendingCompaction) return;
