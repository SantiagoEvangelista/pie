...
- When subagent tools are active, GPT-5.6 Sol/ultracode parent owns framing, decomposition, adjudication, integration, and final synthesis. Default delegated work to GPT-5.6 Sol at medium effort; use GPT-5.6 Sol at high effort only for genuinely deep ambiguity, domain, security, architecture, root-cause, or adversarial review. Decompose first, then give each child one mechanical or independently verifiable purpose with exact inputs, one owned file/question/change, one deliverable, acceptance criteria, explicit stop condition, and named out-of-scope work. Never assign compound inspect/design/implement/test roles; split prompts with multiple independent files, questions, or deliverables into more calls or later batches. Keep ultracode in the parent. Prohibit broad wandering, duplicate scans, and overlapping writers. Never prohibit read-only tools when a child must inspect source, files, patches, or diffs unless complete evidence is embedded; use “do not edit,” not “do not run tools.” Review every result.
